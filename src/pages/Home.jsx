...
const Home = () => {
    ...
    return (
        <div className={`relative gradient-dark w-full min-h-screen flex flex-col justify-center transition-all duration-200 items-center`}>
            <img className={`absolute top-5 md:left-50 md:top-15 ${isFocused ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4 scale-75'}`} src={ProConnectTexto} alt="Logotipo Pro Connect" />
            ...
            <div className={`w-full flex justify-center transition-all duration-300 ease-out ${isFocused ? 'absolute top-15 md:top-4 left-1/2 -translate-x-1/2' : 'relative'}`}>
                <div className="w-full">
                    <Input value={search} onChange={setSearch} onFocus={() => setIsFocused(true)} onBlur={() => { if (!search) setIsFocused(false) }} />
                    <p className={` md:mt-3 text-[16px] text-white transform transition-all duration-300 ease-out text-center `}>Mostrar todos os resultados</p>
                </div>
            </div>
            <img className={`absolute bottom-6 mx-auto w-[286px] ${isFocused ? 'opacity-0 -translate-y-4 scale-75' : 'opacity-100 translate-y-0'}`} src={ProConnectLogo} alt="Logotipo Pro Connect"/>
